...
% V(1) = (k/k_f)*0.2 + 0.1;
% if k > floor(k_f/3) && k < floor(k_f/3)*1.05
%     W(1) = 0.25;
% elseif k > 2*floor(k_f/3) && k < 2*floor(k_f/3)*1.03
%     W(1) = 0.25;
% else
% %     W(1) = -(k/k_f)*0.03 + ((k-floor(k_f/3))/k_f)*0.15;
%     W(1) = -0.01/5;
% end
W(1) = 0.2;
V(1) = 0.4;
% W(1) = 1*D2R -(k/k_f)*0.03*D2R;
% W(1) = D2R*max(0.1,0.5-0.5*sqrt((k/k_f)));
% W(1) = 0.1;
